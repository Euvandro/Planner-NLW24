...
export default function Index() {
    ...
    function handleNextStepForm() {
        ...
        Alert.alert("Nova viagem", "Confirmar viagem?", [{text: "Não", style: "cancel"}, {text: "Sim", onPress: createTrip}])
    }
    ...
    function handleAddEmail() {
        if (!validateInput.email(emailToInvite.toLocaleLowerCase())) {
            return Alert.alert("Convidado", "E-mail inválido!")
        }

        const emailAlreadExists = emailsToInvite.find((email) => email === emailToInvite.toLocaleLowerCase());

        if (emailAlreadExists) {
            return Alert.alert("Convidado", "Este e-mail já foi adicionado!")
        }

        setEmailsToInvite((prevState) => [...prevState, emailToInvite.toLocaleLowerCase()])
        setEmailToInvite("");
    }
    ...
    async function createTrip(){
        try{
            ...
            Alert.alert("Nova viagem", "Viagem criada com sucesso!", [{text: "Ok. Continuar", onPress: () => saveTrip(newTrip.tripId)}])
    ...
    return (
        <View className='flex-1 justify-center items-center px-5'>
            ...
            <Modal title='Selecionar convidados' subtitle='Os convidados irão receber e-maisl para confirmar a participação na viagem.' visible={showModal === MODAL.GUESTS} onClose={() => setShowModal(MODAL.NONE)}>
